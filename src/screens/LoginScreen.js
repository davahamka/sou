...
import {API_KEY, USERS_LOGIN} from '../utils/api';

async function postLoginAPI(data) {
  try {
    const response = await axios.post(API_KEY + USERS_LOGIN, data);
    return response;
  } catch (err) {
    return err;
  }
    ...
    <SafeAreaView>
      ...
      <View style={tailwind('p-4')}>
        ...
        <TouchableOpacity
          color="#0A57FF"
          style={tailwind(
            'py-3 rounded bg-white border border-metrocolor-primary',
          )}
          onPress={() => {
          ...
          <Text
            style={tailwind(
              'text-metrocolor-primary text-center font-bold text-lg',
            )}>
            Register
          </Text>
        </TouchableOpacity>
